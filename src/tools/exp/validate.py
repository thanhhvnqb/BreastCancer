...
def validate(args):
    ...
    with torch.no_grad():
        ...
        input = torch.randn(
            (args.validation_batch_size,) + tuple(data_config["input_size"])
        ).to(device)
        if args.channels_last:
    ...
    metric_results = exp.compute_metrics(targets, preds, sample_weights=sample_weights)
