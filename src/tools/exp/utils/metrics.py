...
def compute_metrics_over_thresholds(
    preds, gts, thresholds=np.linspace(0, 1, 101), eps=1e-3
):
    f1scores = []
    precisions = []
    recalls = []
...
def compute_best_metrics(cancer_p, cancer_t):
    ...
    i = f1scores.argmax()
    f1score, precision, recall, threshold = (
        f1scores[i],
        precisions[i],
        recalls[i],
        thresholds[i],
    ...
    specificity = ((cancer_p < threshold) & ((cancer_t <= 0.5))).sum() / (
        cancer_t <= 0.5
    ).sum()
    sensitivity = ((cancer_p >= threshold) & ((cancer_t >= 0.5))).sum() / (
        cancer_t >= 0.5
    ).sum()
...
def print_all_metric(valid_df):
    ...
    for site_id in [0, 1, 2]:
        ...
        gb = (
            site_df[["patient_id", "laterality", "cancer_t", "cancer_p"]]
...
def compute_all(df, plot_save_path):
    ...
    gb = (
        df[["site_id", "patient_id", "laterality", "cancer_t", "cancer_p"]]
        .groupby(["patient_id", "laterality"])
        .mean()
    )
...
def plot_pr_curve(df, plot_save_path):
    ...
    i = f1scores.argmax()
    f1score_max, precision_max, recall_max, threshold_max = (
        f1scores[i],
        precisions[i],
        recalls[i],
        thresholds[i],
